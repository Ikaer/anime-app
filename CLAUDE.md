...
- `recommendations.json` — cached recommendations feed data (crowd/AniList seeds + hydrated candidates); the code constant is `RECOMMENDATIONS_FILE`.
- `animes_anilist_meta.json` — AniList catalog metadata (tags + staff + banner) keyed by MAL id (see "AniList tags + staff integration"). The code calls it `anilistMeta`.
