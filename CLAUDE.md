...
- `registry.json` — the identity spine: `Record<canonicalId, SourceIds>` (the crosswalk of provider ids — `mal`/`anilist`/`simkl`/…). Every write path resolves-before-mint against this file (`resolveCanonicalId(s)` in `store/index.ts`), so a rebuild never reattaches durable user data to the wrong title.
...
- **The reco engine is the one deliberately MAL-keyed exception.** `computeFeed`/`computeSimilarTo` in [src/lib/reco/engine.ts](src/lib/reco/engine.ts) build an internal `Map<number, AnimeRecord>` keyed by `crosswalk.mal` (coerced via `toNum`), because MAL/AniList crowd edges, suggestions, and `cache/recommendations.json` all arrive as MAL ids. Only the *outward* edges of the engine (each `RecommendationItem`'s `.id`, hidden/feedback exclusion checks) are canonical — see docs/PROVIDER-FREE-CUTOVER.md "Risks".
...
Ranking + fetch logic is in [src/lib/reco/engine.ts](src/lib/reco/engine.ts); endpoints under `src/pages/api/anime/recommendations/` (GET feed, POST refresh via SSE, POST/DELETE `feedback/[id]`).
...
- Reco sources: `anilistTags` AND `anilistStaff` are `MetaField`s (these two keep the `Tags` name — they really are tag- and staff-specific, and `anilistTags` is persisted in the URL weights param) in [src/lib/reco/engine.ts](src/lib/reco/engine.ts), both following the exact `genre`/`studio` IDF-weighted-affinity pattern (tags = plain tag-name list; staff = stable AniList staff `id`, so a shared director/composer is a rare, strong signal; `role` feeds the "Pourquoi?" explain). Neither folds AniList's `rank` into scoring yet. Both ship at weight `0` by default in [src/lib/reco/weights.ts](src/lib/reco/weights.ts) until a sync has populated coverage.
...
[src/lib/store/index.ts](src/lib/store/index.ts) uses Node.js `fs`/`path` (via `jsonStore.ts`) and must never be bundled client-side. Only **pages** (`getServerSideProps`, API routes) import it, and always as values, since they run server-side. Client **components** never need it — they get their types (`AnimeRecord`, `UserAnimeStatus`, etc.) from [@/models/anime](src/models/anime/index.ts), which has no `fs` dependency and is safe to import as values from either side.
