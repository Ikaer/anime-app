...
- Sync is **one-way (SIMKL → app), personal library only** (statused anime), following SIMKL's two-phase model (`docs/simkl/apirules.md`): initial `/sync/all-items/anime` (plain — NOT `extended=ids_only`, which strips per-item status/rating/progress; the plain call already returns those plus full `ids` incl. `mal`, verified against a live account), then `/sync/activities` + `date_from` deltas, with `simkl_sync_checkpoint.json` holding the `anime.all` watermark. Deletion reconciliation diffs `extended=simkl_ids_only` against the local store. Orchestration in [src/lib/simklSync.ts](src/lib/simklSync.ts); auth/state/watermark in [src/lib/simkl.ts](src/lib/simkl.ts); endpoints under `src/pages/api/anime/simkl/` (`auth`, `sync`). **Writes to SIMKL are limited to ONE narrow carve-out: user-initiated ratings** pushed from the Tier list board (see below). Nothing else is ever written to SIMKL — sync remains one-way SIMKL → app.
...
- **SIMKL ratings type-key is self-correcting.** SIMKL's `/sync/ratings` example buckets by `movies`/`shows`, but anime are first-class in the read API — the correct ratings bucket isn't documented. `pushSimklRating` tries a bucket by `media_type`, and if SIMKL echoes the item under `not_found`, retries the other bucket; the full request + response is logged (`[simkl-rating]`) for the first live audit.
