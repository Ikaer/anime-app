...
Its header carries the same split as `/` (see "Key data flow" above): sidebar narrows, header lays out. `DisplaySection variant="inline"` (cards per row) and the "show all explains" toggle sit in the existing `.reco-header` next to the title/count rather than in a sidebar `Affichage` section — but this page mounts **no** `SortOrderSection`, because the feed's order *is* the affinity ranking and a sort control would contradict it. That is why it reuses the inline variant directly instead of `AnimeListHeader`, which is sort-shaped and carries `/`'s title-less count row.
