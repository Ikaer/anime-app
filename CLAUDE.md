...
- **Discrepancy detection** ([src/lib/simklCompare.ts](src/lib/simklCompare.ts), client-safe/pure) compares MAL vs SIMKL status/score/progress and flags presence-on-one-side. Surfaced on the main page as a per-card `SimklDiscrepancyBadge` + a `discrepanciesOnly` (`disc`) URL filter, and on a dedicated read-only [discrepancies page](src/pages/discrepancies.tsx) (route `/discrepancies`) rendering a MAL-vs-SIMKL comparison table (fetches `?discrepancies=true&limit=all`).
...
- Storage: `animes_anilist_meta.json` (`AniListMetaEntry`, keyed by canonical id), joined onto the record in `getAnimeForDisplay()` exactly like `animes_simkl.json`. Holds `tags`, `staff` (`AniListStaffEntry[]`, top-15 by relevance) AND `banner_image`. The latter two are optional so pre-existing entries stay valid — **a `undefined` field is the backfill signal**. `banner_image` is therefore written as an explicit `null` when AniList has none (common), so an absent banner never re-queues forever.
...
### Internationalization (switchable FR/EN)
