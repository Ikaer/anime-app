...
**`node scripts/backtest-reco.js --cutoff YYYY-MM-DD` is how a ranking change gets justified** — see "Measuring a ranking change" below. Its sibling `node scripts/probe-affinity.js --season YYYY-season` does the same job for the main list's « Recommandé » mark; both MEASURE rather than assert, and neither is a test.
...
- `user/reco_profiles.json` — reco profiles: a bare `RecoProfile[]`, hand-tuned weightings a box points at through `Box.profileId` (docs/recoProfiles/; PLAN.md records what each phase decided and measured; edited on `/profiles`, see below). Store `reco/profiles.ts`, shape and resolver the client-safe `reco/profileWeights.ts`. ⚠️ Durable user data like `boxes.json`. A deleted profile leaves the box's `profileId` in place, inert; `mintProfileId` reserves every id a box still names, so a re-created profile of the same name can never silently re-bind it
...
### « Recommandé » — the taste mark on the main list

Two badges on the card in [AnimeCardView](src/components/anime/AnimeCardView.tsx) — ★ *Recommandé* and ☆ *À regarder* — plus an `affinity` sort column, computed by [src/lib/reco/affinity.ts](src/lib/reco/affinity.ts) and attached by [api/anime/animes](src/pages/api/anime/animes/index.ts). It answers the question a season start asks: `catalog.mean` is null on every row of a season that has not aired, so the list has nothing to say about 90 new titles.

- **It is a FOURTH ranker, and reusing the feed was never an option.** `computeFeed` and `computeAnchored` build their candidate set out of **crowd edges**; an unaired title has none, so it is not thin there, it is *structurally absent*. So the score is metadata-only (genre, tags above the rank floor, T1-inclusive staff, studio, netted against `rejection`) — every field of it available the day a title is announced. Nothing here calls a provider.
- **Measured before it was built, then re-measured through the shipped module** — `node scripts/probe-affinity.js --season 2024-fall`, which replays a past season exactly as `backtest-reco.js` replays the feed (profile rebuilt from what the owner had watched by the cutoff, SIMKL `watched_at` as the clock). Over six seasons of TV titles, 420 candidates, 40 held-out favourites: **p90 badged 24% of the season and caught 60%**, p95 12% / 48%, p97 7% / 38%. Unlike the feed's harness it needs **no cache pruning** — this ranker never reads `cache/recommendations.json`, so no seed leaks back in.
- ⚠️ **Those numbers are a ceiling, because AniList tags accumulate as a show airs.** Live-measured: an aired season carries a median 12 tags (8 above rank 60) and 20 staff credits; the NEXT season carries **4 / 1 / 8**, with 32 of 72 TV titles holding no strong tag at all. `--thin` truncates candidates to that density and costs about a fifth of the recall. Two consequences: the mark is *sparse at announcement and fills in over the first weeks* (free — the list re-scores every request), and the tier cuts drift downward the thinner the population is. Live on the store, the four most recent seasons' median affinity ran 0.044 → 0.096 → 0.202 → 0.302 as their density ran 11 → 18 → 25 → 27; that is the metadata arriving, not taste changing.
- ⚠️ **The floor is mandatory here, not tidy.** `MATCH_DENOM_FLOOR` + `flooredFieldMatch` moved from `boxes.ts` into [scoring.ts](src/lib/reco/scoring.ts) so both rankers share one measured constant (the feed still does NOT use it — there `crowd` anchors the ranking, so a sparse title cannot ride one tag to the top). Season-start titles are *the* evidence-poor population, so without the floor the badge would preferentially mark the thinnest new entries — the *LONA* failure the box ranker measured.
- ⚠️ **The tier cut is a percentile of the SCOREABLE UNSEEN CATALOG — a fixed population, never the current view.** A cut over whatever is filtered on screen badges the same share of every screen and therefore asserts nothing; a weak season must be allowed to come out with two marks. And "scoreable" is load-bearing: ~40% of unseen rows carry no AniList entry and score a flat 0, so a percentile over all of them lands inside a block of zeros and badges everything. That is `popularityScale`'s lesson in threshold form, and it is pinned in [tests/reco/affinity.test.ts](tests/reco/affinity.test.ts).
- **Eligibility follows the feed's `SEEN_STATUSES`, not "has any status"** — `plan_to_watch` is a title you flagged, not one you judged, and both `computeFeed` and `/mix` count it as unseen. Also excluded: hidden, 👎, and **premature sequels** (a season is mostly sequels, and ★ on part 3 of a show never started would discredit the mark on sight). ⚠️ **It is exported, and the reco-profile preview's catalog pool is exactly it** (`isUnseenCandidate` → `isScoreable` → `isPrematureSequel`, composed by `buildUnseenPool` in the mark's own order — as TWO predicates, because the first alone decides `coverage.unseen`). Pinned by comparing the two populations. The preview borrows the population only: no profile ever reaches the mark (docs/recoProfiles/DESIGN.md §10).
- ⚠️ **MAL member counts are NOT in the score, and that is a measurement rule.** `num_list_users` is the one crowd figure that exists pre-air (2026-fall TV: zero titles with a `mean`, but a median 5,006 members, p90 35,463) and it is real anticipation — so `buildAnticipationIndex` surfaces it as a 🔥 chip **in the score's slot, only while there is no `mean`**, ranked **within the title's own season** (counts grow with age, so cross-season ranking measures age, not hype). It cannot enter the *ranking* because the store keeps only a present-day snapshot: Dandadan carries 1,019,332 members today, 23× its season's median, precisely BECAUSE it was a hit — a backtest would "discover" pure hindsight. Making it scoreable needs a member-count history the store does not keep. Same class as the rejected `num_episodes` knob: a preference the harness cannot falsify.
- **Computed catalog-wide, memoized on the row array's identity** (the `byCredits` / `getFranchiseIndex` WeakMap trick, so it self-invalidates with the store) — the cut needs the whole population, the route returns 200 rows. ~400 ms per store change, then a Map lookup. ⚠️ Marks ride on a **copy** (`AnimeListRow`), never written onto the record: `getAnimeForDisplay()` hands back the shared cache array.
- ⚠️ **The mark is the main list's and is NOT threaded into `/recommendations` or `/mix`**, which render the same `AnimeCardView`. Those already answer "why is this here" with `recoMeta`, and a second verdict derived from different evidence (metadata-only vs crowd-anchored) reads as a contradiction the moment the two disagree. A ★ title therefore shows nothing there — deliberate, not an oversight. Eligibility also excludes a `RatingIntent` (« À revoir » / « Sans avis ») outright rather than relying on those riding on a `completed` status, so moving where the intent is stored cannot put « Recommandé » on a show the owner watched and declined to score.
- **The `why` tooltip resolves staff ids to names.** `FIELD_EXTRACTORS.anilistStaff` keys on the AniList staff **id** — right for the math, unreadable in a badge, and live-verified rendering as `anilistStaff: 120066, 100070` before `labelFor` fixed it.
- **`affinity` sort does not go through `sortAnimeRecords`**: the score is a per-request sibling of the record, not a catalog field, and threading the index into the shared sorter would hand every other caller a parameter it has no use for. Unscored rows sort last either way, matching that function's `numOrNull` rule.
...
- **The fetch is its own module, and edges are cached per anchor** ([reco/mixFetch.ts](src/lib/reco/mixFetch.ts): `boxAnchorIds`, `loadMixEdges`, process-lifetime, no TTL). It was the route's half until `scripts/probe-profile.js --rank --pool anchored` needed the exact same anchors and edges — `similarFetch.ts`' reason; the route ([mix.ts](src/pages/api/anime/recommendations/mix.ts)) keeps the request parsing, the weights' precedence and the card projection. The page refetches on every add/remove; without the cache, adding a 5th anchor would re-ask MAL about the other four. Crowd edges move on a scale of months, and only successful fetches are cached, so a dead source retries. MAL costs one request per anchor, AniList one for all of them (`id_in`) — cost scales with anchors *added*, not anchor count. `MAX_MIX_ANCHORS = 12` is a guard on that cost.
...
- ⚠️ **A declaration NEVER adds membership.** `members` is authoritative, `groups` is a lens
  ...
  as one" is what a group delete actually does. ⚠️ **A dead id stays inert only because it is
  never re-minted**: `createGroup` mints through `mintGroupId` (client-safe, in
  [domain/slug.ts](src/lib/domain/slug.ts)), which reserves every id any box still declares —
  `mintProfileId`'s rule. Without it, deleting « Bleach » and creating a new « Bleach » re-issued
  `bleach`, and every box still declaring it silently started collapsing the new group's members.
  `createGroup` takes the boxes as a **required** argument (both callers pass `getBoxes()`) rather
  than reading them itself, because `boxes.ts` imports `groups.ts` and the reverse import would be
  the cycle `slug.ts` exists to avoid. Pinned in [tests/domain/slug.test.ts](tests/domain/slug.test.ts).
...
- **A box's reco profile re-weights BOTH** (`Box.profileId` → `getBoxProfile`; docs/recoProfiles/,
  PLAN.md phase 4). Each ranker resolves it over its OWN base — `BOX_WEIGHTS` in the fill loop,
  `ANCHORED_WEIGHTS` on the recos tab — through `resolveProfile`, which zeroes `anilistStaff`
  whenever a staff craft family is on. The families are scored by `staffFields.ts`' shared
  `buildFamilyTerms` / `matchFamilyTerms`, only for non-zero families, so a box with no profile
  ranks byte-identically (`probe-box.js --box all` diffed clean). ⚠️ **Through `denomFor`, never
  plain `fieldMatch`** — `computeAnchored` scores its metadata fields unfloored, which on a
  near-binary family is the uncorrected ~3× scale `PROFILE_DENOM` exists to remove. ⚠️ **In the
  same explain as every source**: `RecoContribution.source` is `RecoSource | StaffFamily`, the
  anchored ranker sums and lists through ONE pass (`scoreWithBreakdown`, which the feed uses too)
  and the fill loop pushes each family into `matched` beside its score — a family scored but not
  listed is `projectWhy`'s under-reporting bug. `mix?box=` resolves base < profile < URL
  (`resolveProfileOver`, which re-applies the zeroing after a hand-typed `w=anilistStaff:…`) and
  echoes `profile.base`, the weights a control must encode `w` against or a slider set to the
  anchored default snaps back to the profile. `rankBoxCandidates` takes the profile as an option
  rather than reading it — `profiles.ts` imports `boxes.ts`. Measured with `probe-profile.js
  --rank`: `staffDirector: 1` on `Bancal et je l'assume` turns 7 of the fill loop's top 12 into
...
- **`POST /api/anime/profiles/preview` is the tuning instrument** ([reco/profilePreview.ts](src/lib/reco/profilePreview.ts),
  ...
  loop unchanged; `anchored` is the recos tab through `mixFetch` (the only one calling MAL /
  AniList). It ships the §8 diagnostic from [reco/profileDiagnostic.ts](src/lib/reco/profileDiagnostic.ts)
  — per family, people and recurrence across UNITS with a verdict (`empty | retrieval | axis`) —
  because a craft slider on a small box retrieves "more by these people" rather than learning an
  axis, and only this block says which. `probe-profile.js` reads both modules rather than copies;
  the `/profiles/[id]` page (below) is its one UI caller.
  ⚠️ `preview` is a reserved profile id: the static route would shadow `profiles/[id]`.
...
### "/profiles" — reco profiles, tuned against a box

[/profiles](src/pages/profiles/index.tsx) (list, create from a `PROFILE_PRESETS` key) and
...
- ⚠️ **The sliders are sparse-aware, NOT `RecoWeightsSection`.** A profile is sparse by intent: a
  row is inherited (the pool's base, muted), set, or reset — and reset DELETES the key
...
- **Attach is in box EDITION** ([BoxProfileControl](src/components/anime/boxes/BoxProfileControl.tsx)),
  présentation states it read-only (a deleted profile reads « Profil supprimé (inactif) »), the
  recos tab names the profile it ranked with, and the profile page attaches to the box it is being
  tested on. The box recos tab deliberately has NO sliders: one tuning surface per weighting.
...
- **Reco profiles are READABLE, never writable** (docs/recoProfiles/DESIGN.md §9), and the boxes
  carve-out is deliberately not precedent: a profile silently re-weights every ranking that
  follows, so a model able to set one would be tuning the answer it is about to give —
  `addSeedMute`'s objection. `createProfile` / `updateProfile` / `deleteProfile` are blocked by
  name, and so is `setBoxProfile` — which lives in the otherwise-importable `boxes.ts` and is a
  separate function from `updateBox` precisely so it CAN be named there. What the tools DO with
  one: `box_candidates` and `get_box` rank through the box's attached profile and return it as
  `profile` (id, name, the sparse weights), and their descriptions say a craft-family hit in
  `matched` is "more by this person", the owner's weighting — not evidence of what the box is
  about. Stated rather than applied silently, for `projectWhy`'s reason.
...
- **What it found (2026-09-11): the ~10s was NOT the app.** The slow `/boxes` load logged `queueMs: 6031`, `ttfbMs: 38`, `serverMs: 18` on a fresh connection — the time went on resolving the hostname `syno` before the request left the browser. On the office PC no DNS server knows that single-label name (NordLynx's, 8.8.8.8 and the router all time out, 7-11s) and Windows only then falls back to LLMNR/NetBIOS (15ms); the answer caches for about a minute, hence "fine, then one slow load". Fix is client-side (a hosts entry or the IP), not code. Check `queueMs` before chasing a slow load server-side.
