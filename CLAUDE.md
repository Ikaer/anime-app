...
- `user/reco_profiles.json` — reco profiles: a bare `RecoProfile[]`, hand-tuned weightings a box points at through `Box.profileId` (docs/recoProfiles/, being built — PLAN.md tracks the phases). Store `reco/profiles.ts`, shape and resolver the client-safe `reco/profileWeights.ts`. ⚠️ Durable user data like `boxes.json`. A deleted profile leaves the box's `profileId` in place, inert; `mintProfileId` reserves every id a box still names, so a re-created profile of the same name can never silently re-bind it
